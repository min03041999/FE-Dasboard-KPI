...
const ProductionScreen = () => {
    ...
    const [navigate, setNavigate] = useState({ factory: FACTORY, floor: "", line: "", section: "" });
    ...
    let OUTPUT_BY_THE_HOURS = navigate?.section === "stitching" ? actualStitching : actualAssembly;
    ...
    let WORKER = navigate?.section === "stitching" ? worker.stitching : worker.assembly;
    ...
    const TARGET = navigate?.section === "stitching" ? target.targetStitching : target.targetAssembly;
    ...
    // SET FOR SLIDE 1
    const SET_FULL_SCREEN_LAPTOP = screenHeight > 730 ? { height: `${screenHeight / 3 - 16}px` } : { height: "300px" };
    const SET_FULL_SCREEN_LAPTOP_ = { ...SET_FULL_SCREEN_LAPTOP, height: parseFloat(SET_FULL_SCREEN_LAPTOP.height) / 2 - 8 };

    // SET CHART
    const SET_HEIGHT_CHART = screenHeight > 730 ? (screenHeight / 3) - 51 : 300 - 51;

    return (
        <Box component={"div"} className='production-screen'>
            <Box component={"div"} sx={screenHeight > 730 && screenHeight < 780 ? { position: "relative", height: `${screenHeight / 3}px` } : { position: "relative", height: "100%" }}>
                <Breadcrumb>
                    Prod. KPI Dashboard
                </Breadcrumb>
                <Navigation navigate={navigate} setNavigate={setNavigate} date={date} setDate={setDate} />
            </Box>

            <Box component={"div"} className='production-screen-body'>
                {navigate.line === "" ? (
                    <Grid container spacing={{ xs: 2, md: 2 }} columns={{ xs: 4, sm: 4, md: 8, lg: 12 }}>
                        <Grid item xs={4}>
                            <EFFByFloor customStyle={SET_FULL_SCREEN_LAPTOP} setHeightChart={SET_HEIGHT_CHART} header={"EFFICIENCY BY FLOOR"} type={"eff"} typeChart={"ColumnMixed"} data={transformed_eff} />
                        </Grid>
                        <Grid item xs={4}>
                            <RFTByFloor customStyle={SET_FULL_SCREEN_LAPTOP} setHeightChart={SET_HEIGHT_CHART} header={"RFT BY FLOOR"} type={"rft"} typeChart={"ColumnMixed"} data={transformed_rft} />
                        </Grid>
                        <Grid item xs={4}>
                            <Grid container spacing={2}>
                                <Grid item xs={6} sm={6} md={6}>
                                    <AnalyzerTarget customStyle={SET_FULL_SCREEN_LAPTOP_} header={"EFFICIENCY"} target={`BASE LINE: ${IS_TARGET_EFF_RFT.floor.eff}%`} data={"70%"} />
                                </Grid>
                                <Grid item xs={6} sm={6} md={6}>
                                    <AnalyzerTarget customStyle={SET_FULL_SCREEN_LAPTOP_} header={"EFT"} target={`TARGET: ${IS_TARGET_EFF_RFT.floor.rft}%`} data={"70%"} />
                                </Grid>
                                <Grid item xs={6} sm={6}>
                                    <AnalyzerTotalOutput customStyle={SET_FULL_SCREEN_LAPTOP_} header={"TOTAL OUTPUT"} />
                                </Grid>
                                <Grid item xs={6} sm={6} md={6}>
                                    <AnalyzerTopLine customStyle={SET_FULL_SCREEN_LAPTOP_} header={"STOPLINE"} data={stopLineList} />
                                </Grid>
                            </Grid>
                        </Grid>
                        <Grid item xs={4}>
                            <OutPutByFloor customStyle={SET_FULL_SCREEN_LAPTOP} setHeightChart={SET_HEIGHT_CHART} header={"OUTPUT BY LINE"} data={OUTPUT_BY_FLOOR} />
                        </Grid>
                        <Grid item xs={4} >
                            <HourlyOutputByFloor customStyle={SET_FULL_SCREEN_LAPTOP} setHeightTable={SET_HEIGHT_CHART} header={"HOURLY OUTPUT BY FLOOR"} data={tableData} />
                        </Grid>
                        <Grid item xs={4} sx={SET_FULL_SCREEN_LAPTOP}>
                            <AttendanceByFloor customStyle={SET_FULL_SCREEN_LAPTOP} setHeightChart={SET_HEIGHT_CHART} header={"ATTENDANCE BY FLOOR"} data={chartData} />
                        </Grid>
                    </Grid>
                ) : (
                    <Slick setDot={false} setArrow={true} swipe={true}>
                        <div>
                            <Grid container spacing={{ xs: 2, md: 2 }} columns={{ xs: 4, sm: 4, md: 8, lg: 12 }}>
                                <Grid item xs={4} >
                                    <EFFByTheHour customStyle={SET_FULL_SCREEN_LAPTOP} setHeightChart={SET_HEIGHT_CHART} header={"EFFICIENCY BY THE HOUR"} type={"eff"} section={navigate.section} data={RFT_BY_THE_FLOOR} />
                                </Grid>
                                <Grid item xs={4}>
                                    <RFTByTheHour customStyle={SET_FULL_SCREEN_LAPTOP} setHeightChart={SET_HEIGHT_CHART} header={"RFT BY THE HOUR"} type={"rft"} section={navigate.section} data={EFF_BY_THE_FLOOR} />
                                </Grid>
                                <Grid item xs={4}>
                                    <ModelRun customStyle={SET_FULL_SCREEN_LAPTOP} header={"MODEL RUN BY LINE"} targetStitching={target.targetStitching} targetAssembly={target.targetAssembly} data={SHOE_DATA} />
                                </Grid>
                                <Grid item xs={4}>
                                    <OutPutTheHour customStyle={SET_FULL_SCREEN_LAPTOP} setHeightChart={SET_HEIGHT_CHART} header={"OUTPUT BY THE HOUR"} target={TARGET} plannedWorkingHours={8} data={OUTPUT_BY_THE_HOURS} />
                                </Grid>
                                <Grid item xs={4}>
                                    <PPHByTheHour customStyle={SET_FULL_SCREEN_LAPTOP} setHeightChart={SET_HEIGHT_CHART} header={"PPH BY THE HOUR"} data={OUTPUT_BY_THE_HOURS} worker={WORKER} />
                                </Grid>
                                <Grid item xs={4}>
                                    <Grid container spacing={2}>
                                        <Grid item xs={6} sm={6} md={6}>
                                        ...
                                        <Grid item xs={6} sm={6} md={6}>
                                            <AnalyzerAttendance customStyle={SET_FULL_SCREEN_LAPTOP_} header={"ATTENDANCE"} />
                                        </Grid>
                                    </Grid>
                                </Grid>
                            </Grid>
                        ...
                        <div>
                            <Grid container spacing={{ xs: 2, md: 2 }} columns={{ xs: 4, sm: 4, md: 8, lg: 12 }}>
                                <Grid item xs={12}>
                                    <Top3Defect header={"STOPLINE TOP 3 DEFECT"} data={TOP3_DEFECT} setHeightChart={SET_HEIGHT_CHART} />
                                </Grid>
                            </Grid>
                        </div>
