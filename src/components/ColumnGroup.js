...
const ColumnGroup = (props) => {
    ...
    return (
        <ResponsiveContainer width="100%" height={setHeightChart}>
            <BarChart
                {...COLUMN_GROUP_CONFIG.BarChart} data={data}
            >
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis {...COLUMN_GROUP_CONFIG.XAxis} dataKey="line" />
                <Bar dataKey="stitching" fill="#118dff" {...COLUMN_GROUP_CONFIG.Bar}>
                    <LabelList
                        dataKey="stitching"
                        {...COLUMN_GROUP_CONFIG.LabelList}
                    />
                </Bar>
                <Bar dataKey="assembly" fill="#12239e" {...COLUMN_GROUP_CONFIG.Bar}>
